...
def build_policy_parameter_register() -> pd.DataFrame:
    ...
    return out.sort_values(
        ['record_type', 'fallback_hierarchy', 'fallback_priority', 'group', 'parameter'],
        na_position='last',
...
def build_features(raw: pd.DataFrame) -> pd.DataFrame:
    d = raw.copy()
    d['undrawn'] = d['limit'] - d['drawn']
    d['utilisation'] = _div(d['drawn'], d['limit']).clip(0, 1.5)
    d['margin'] = _div(d['ebitda'], d['revenue'])
    d['dscr'] = _div(d['ebitda'], d['debt_service'])
    d['leverage'] = _div(d['net_debt'], d['ebitda'])
    d['working_capital_ratio'] = _div(d['current_assets'], d['current_liabilities'])
    d['quick_ratio'] = _div(d['current_assets'] - d['inventory'], d['current_liabilities'])
    d['collateral_coverage'] = _div(d['collateral'], d['ead'])
    d['receivables_days'] = (_div(d['receivables'], d['revenue']) * 365).round(1)
    d['inventory_days'] = (_div(d['inventory'], d['revenue'] * .65) * 365).round(1)
    d['payables_days'] = (_div(d['payables'], d['revenue'] * .7) * 365).round(1)
    d['cash_conversion_cycle'] = d['receivables_days'] + d['inventory_days'] - d['payables_days']
    return d
...
def build_outputs(f: pd.DataFrame) -> dict[str, pd.DataFrame]:
    if PIPELINE_KIND == 'financial':
        flags = f[['borrower_id', 'facility_id', 'segment', 'industry', 'dscr', 'leverage', 'quick_ratio']].copy()
        flags['qualitative_risk_flag'] = flags.apply(lambda r: '; '.join([n for n, c in {'low_dscr': r.dscr < 1.25, 'high_leverage': r.leverage > 3.5, 'weak_liquidity': r.quick_ratio < 1}.items() if c]) or 'no_major_flag', axis=1)
        diag = f.groupby(['segment', 'industry'], as_index=False).agg(avg_dscr=('dscr', 'mean'), avg_margin=('margin', 'mean'), avg_leverage=('leverage', 'mean'), facilities=('facility_id', 'count'))
        diag['trend_risk'] = diag.avg_dscr.apply(lambda v: 'elevated' if v < 1.25 else 'stable')
    ...
    if PIPELINE_KIND == 'industry':
        g = f.groupby('industry', as_index=False).agg(avg_dscr=('dscr', 'mean'), avg_margin=('margin', 'mean'), avg_utilisation=('utilisation', 'mean'), exposure=('ead', 'sum'), facilities=('facility_id', 'count'))
        g['industry_risk_score'] = (5 - g.avg_dscr.clip(0, 2.5) + g.avg_utilisation * 2 - g.avg_margin.clip(0, .3)).clip(1, 5)
        down = g[['industry', 'industry_risk_score']].copy()
        down['pd_overlay_multiplier'] = 1 + down.industry_risk_score * .08
        down['lgd_overlay_addon'] = down.industry_risk_score * .015
        soft = g[['industry', 'avg_utilisation', 'avg_margin']].copy()
        soft['market_softness_band'] = soft.apply(lambda r: 'soft' if r.avg_utilisation > .75 or r.avg_margin < .09 else 'normal', axis=1)
        conc = g[['industry', 'exposure', 'facilities']].copy()
        conc['portfolio_share'] = conc.exposure / conc.exposure.sum()
        conc['concentration_flag'] = conc.portfolio_share.apply(lambda v: 'review' if v > .30 else 'within_demo_limit')
    ...
    if PIPELINE_KIND == 'pd':
        w = f.copy()
        w['pd_estimate'] = (w.pd * (1 + (1.25 - w.dscr).clip(lower=0) * .20 + w.utilisation.clip(0, 1) * .10)).clip(.002, .35)
        w['score'] = (720 - w.pd_estimate * 2500 + w.dscr.clip(0, 3) * 18 + w.margin.clip(0, .4) * 100 - w.leverage.clip(0, 8) * 9).round(0)
        w['risk_grade'] = w.pd_estimate.apply(_grade)
        out = w[['borrower_id', 'facility_id', 'segment', 'industry', 'product_type', 'score', 'risk_grade', 'pd_estimate', 'dscr', 'utilisation']].copy()
        out['policy_decision'] = w.apply(_decision, axis=1)
    ...
    if PIPELINE_KIND == 'lgd':
        w = f.copy()
        w['net_collateral'] = w.collateral * (1 - w.haircut)
        w['workout_cost'] = w.ead * .06
        w['net_recovery'] = (w.net_collateral - w.workout_cost).clip(lower=0)
        w['lgd_final'] = (1 - w.net_recovery / w.ead.replace(0, pd.NA)).fillna(1).clip(.10, .90)
        w['value_decline_proxy'] = w.haircut.clip(0, .45)
        w['cashflow_weakness_proxy'] = ((1.25 - w.dscr) / 1.25).clip(lower=0, upper=1)
        w['recovery_delay_proxy'] = ((w.maturity - 2.5) / 3).clip(lower=0, upper=1)
        w['downturn_scalar'] = (1 + 0.30 * w.value_decline_proxy + 0.10 * w.cashflow_weakness_proxy + 0.08 * w.recovery_delay_proxy).clip(1.00, 1.45)
        w['downturn_lgd'] = (w.lgd_final * w.downturn_scalar).clip(upper=.95)
    ...
    if PIPELINE_KIND == 'ead':
        w = f.copy()
        mp = {'term_loan': 0, 'overdraft': .65, 'commercial_mortgage': .20, 'revolving_working_capital': .55, 'trade_finance': .45, 'guarantee': .30}
        w['base_ccf'] = w.product_type.map(mp).fillna(.4)
        w['utilisation_uplift'] = w.utilisation.apply(lambda v: .10 if v > .85 else .05 if v > .65 else .02)
        w['downturn_ccf'] = (w.base_ccf + w.utilisation_uplift).clip(upper=1)
        w['ead_central'] = w.drawn + w.undrawn * w.base_ccf
        w['ead_downturn'] = w.drawn + w.undrawn * w.downturn_ccf
    ...
    if PIPELINE_KIND == 'el':
        w = f.copy()
        w['expected_loss_12m'] = w.pd * w.lgd * w.ead
        w['lifetime_pd'] = (w.pd * w.maturity * 1.15).clip(upper=.75)
        w['expected_loss_lifetime'] = w.lifetime_pd * w.lgd * w.ead
        scen = pd.DataFrame([{'scenario': 'base', 'weight': .60, 'pd_multiplier': 1, 'lgd_multiplier': 1}, {'scenario': 'mild_downturn', 'weight': .25, 'pd_multiplier': 1.3, 'lgd_multiplier': 1.08}, {'scenario': 'severe_downturn', 'weight': .15, 'pd_multiplier': 1.7, 'lgd_multiplier': 1.18}])
        scen['scenario_expected_loss'] = scen.apply(lambda r: (w.pd * r.pd_multiplier * w.lgd * r.lgd_multiplier * w.ead).sum(), axis=1)
        scen['weighted_expected_loss'] = scen.scenario_expected_loss * scen.weight
    ...
    w = f.copy()
    w['risk_weight'] = (.45 + w.pd * 8 + w.lgd * .50 + w.maturity * .03).clip(.35, 1.50)
    w['rwa'] = w.ead * w.risk_weight
    w['capital_requirement'] = w.rwa * .105
    w['expected_loss'] = w.pd * w.lgd * w.ead
    w['capital_after_el_adjustment'] = (w.capital_requirement - w.expected_loss).clip(lower=0)
...
def run_pipeline(project_root: Path | str | None = None, persist: bool = True) -> dict[str, object]:
    ...
    raw = load_demo(root / 'data' / 'raw' / 'demo_portfolio.csv')
    feat = build_features(raw)
    outs = build_outputs(feat)
    outs['policy_parameter_register.csv'] = build_policy_parameter_register()
    val = validate_outputs(outs)
    outs['pipeline_validation_report.csv'] = val
    paths = {}
    if persist:
...
def main() -> None:
    ...
    if args.source is not None:
        ...
        result = run_pipeline()
        print(f"{REPO_NAME}: wrote {len(result['outputs'])} output tables")
